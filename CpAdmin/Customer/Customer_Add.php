...
if($PageView == '1'){
    ...
    Customer_Print_Info($PageType,$row);
    Customer_Print_LoginInfo($PageType,$row);
    Customer_Print_SMS_Config($PageType,$row);
    Customer_Print_Photo($PageType,$row);    
    ...
    if($PageType == "Edit"){
        ...
        if(isset($_POST['B1'])){
            Vall($Err,"ThisCustomerEdit",$db,"1",$USER_PERMATION_Edit);
        }
    }elseif($PageType == "Add"){
        Form_Close_New("1","ListCustomer");
        if(isset($_POST['B1'])){
            Vall($Err,"ThisCustomerAdd",$db,"1",$USER_PERMATION_Add);
        }
    }
