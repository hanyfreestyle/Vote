...
###################################################   NF_PrintInput
#################################################################################################################################
function NF_PrintInput($StateType,$Label,$Name,$reqAstrex,$Mass,$req,$Arr=array()) {
    global $AdminWebLang ;
    if(isset($_POST[$Name])) {
    ...
    switch ($StateType) {
        ...
        case 'Numbers':
            ...
            echo '<div class="'.$Arr['Col'].' col-sm-12 col-xs-12 form-group DirRight ">';
            echo  $Print_Label ;
            echo '<input type="number" name="'.$Name.'" id="'.$Name.'" class="TypeText form-control '.$Arr['Dir'].'" 
        value="'.hetsee_2($Name).'" placeholder="'.$Arr['Placeholder'].'" '.$Arr['required'].'  > ';
            echo '</div>';
            FormPrintOneLine(isset($Arr['OnLine']),$Arr);
            break;
        ...
        case 'TextEdit':
            ...
            echo  $Print_Label ;
            echo '<input type="text" name="'.$Name.'" id="'.$Name.'" class="TypeText form-control '.$Arr['Dir'].'"  
            value="'.hetseeEdit_2($Name,$Name).'" placeholder="'.$Arr['Placeholder'].'" '.$Arr['required'].'  > ';
            echo '</div>';
            FormPrintOneLine(isset($Arr['OnLine']),$Arr);
            break;
        case 'Date':
        ...
        case 'TextArea':
            echo '<div class="'.$Arr['Col'].' col-sm-12 col-xs-12 form-group DirRight">';
            echo  $Print_Label ;
            echo '<textarea placeholder="'.$Arr['Placeholder'].'" id="'.$Name.'" '.$Arr['required'].' class="form-control '.$Arr['Dir'].' " name="'.$Name.'" >'.hetsee_2($Name).'</textarea>';
            echo '</div>';
            FormPrintOneLine(isset($Arr['OnLine']),$Arr);
            break;
        case 'TextAreaEdit':
...
###################################################   NF_PrintSelect_2018
#################################################################################################################################
function NF_PrintSelect_2018($StateType,$Label,$Size,$Name,$catTabel,$req,$catid="0",$Arr=array()){
    global $db;
    ...
    */
    if(isset($Arr['Multiple']) and $Arr['Multiple'] == '1'){
    ...
    switch ($StateType) {
        case 'Chosen':
            ...
            //// Start List
            echo '<select name="'.$Name.$MultipleLineArr.'" id="'.$Name.'" '.$Arr['Ajex_01']." ".$MultipleLine.'  class="'.$MyFormStyleForList.'" '.$reqV.'>';
            if(isset($Arr['Label']) and $Arr['Label'] == 'on'){
            ...
            $ListCout = $db->H_Total_Count($MySql);
            if($ListCout >0){
                ...
                for($xx = 0; $xx < count($GroupList); $xx++) {
                    ...
                    if($already){
                      //  echo '<div class="UserGroupName">'.$GroupList[$xx][$SqlPrint_GroupName]."</div>";
                        ...
                        New_Print_Alert("2",$GroupList[$xx][$SqlPrint_GroupName]); 
