...
function LoadTabelData_To_Arr($State,$Tabel_Name,$LoadeFilde="id,name,name_en"){
 ...
 function UnitTypeFilter_Form_From_One($ThisAccountFollow){
    if(count($ThisAccountFollow) >= '1' ){
        $UserPer = " and ( ";
        for ($i = 1; $i <= count($ThisAccountFollow); $i++) {
            if($i == '1'){
...
###################################################    HandleDuplicate
#################################################################################################################################
function HandleDuplicate($Tabel,$Filde,$Post,$id="0",$IdFilde="id",$Arr=array()){
    global $db ;
    global $AdminLangFile ;
...
function Rand_Number_H($num_chars) {
   $ret = "";
...
###################################################    MainConfigSection
#################################################################################################################################
function MainConfigSection_Return($ConfigTabel,$Arr=array()){
    global $row ; 
...
function MainConfigSection($SectionName,$PhotoS='1',$FunArr=array()){
    ...
    $MoreS = array('Col' => "col-md-3",'Placeholder'=> "",'required' => 'required data-parsley-type="digits" ', 'Dir'=> "En" );
...
############################################# GetCatConfig
#################################################################################################################
function Get_Customer_Config($UserId){
    global $db;
    
    
    $row = $db->H_SelectOneRow("SELECT * FROM customer where id = '$UserId' ");
    
    if($row['config'] == ''){
    $ConfigP = array('perpage'=> '10',"order"=>'1');    
    }else{
    $ConfigP =  unserialize($row['config']);    
    }
...
###################################################    MainConfig_AddFilde
#################################################################################################################################
function MainConfig_AddFilde($FileName){
    global $db ;
    $already = $db->H_Total_Count("select * from config_cat where cat_id = '$FileName'");
    if($already == '0'){
        $server_data = array ('id'=> NULL ,
            'cat_id'=> $FileName  ,
            'des'=> ""
        );
        $db->AutoExecute("config_cat",$server_data,AUTO_INSERT);
    }
}
