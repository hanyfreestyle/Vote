...
function Time_Ago_2($date,$Todate) {
   ...
   $periods = array("ثانية","دقيقة","ساعات","يوم","اسبوع","شهر","سنة","decade");
   $lengths = array("60","60","24","7","4.35","12","10");
   $now = $Todate ;
   if($timetype = "1") {
     $unix_date = $date;
...
###################################################   GetARDate
#################################################################################################################################
function GetMonthName($namemonth) {
...
###################################################    ReportPeriod_Month
#################################################################################################################################
function ReportPeriod_Month($Month,$Year){

 $firstDayUTS = mktime (0, 0, 0, date($Month), 1, date($Year));
 $lastDayUTS = mktime (0, 0, 0, date($Month), date('t',$firstDayUTS), date($Year));   
 $CountLoop = date('t',$firstDayUTS);
 $FullDate = array("start"=>$firstDayUTS ,"end"=> $lastDayUTS ,"Loop"=> $CountLoop ); 
